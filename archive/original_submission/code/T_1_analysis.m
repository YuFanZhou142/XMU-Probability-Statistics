...
figure;
subplot(1,2,1);
plot(u1(1:end-1), u1(2:end), '.');
title('u_1(i) vs u_1(i+1) 散点图');
xlabel('u(i)'); ylabel('u(i+1)');

subplot(1,2,2);
plot(u2(1:end-1), u2(2:end), '.');
title('u_2(i) vs u_2(i+1) 散点图');
xlabel('u(i)'); ylabel('u(i+1)');

% ===== 均值与方差 =====
mean1 = mean(u1); var1 = var(u1);
mean2 = mean(u2); var2 = var(u2);

fprintf('第一组：均值 = %.4f，方差 = %.4f\n', mean1, var1);
fprintf('第二组：均值 = %.4f，方差 = %.4f\n', mean2, var2);
fprintf('理论参考值：均值 = 0.5，方差 = 0.0833\n');
